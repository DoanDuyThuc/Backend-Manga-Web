...
const CreateTruyenService = async ({
  truyen_ma,
  truyen_ten,
...
const GetAllTruyenService = async ({
  ...
  try {
    ...
    const {
      rows: truyenList,
      count
    } = await db.Truyen.findAndCountAll({
      attributes: ['id', 'truyen_ma', 'truyen_ten', 'truyen_hinhanhdaidien', 'truyen_duyet', 'truyen_tacgia', 'createdAt', [Sequelize.literal(`(
                    SELECT COUNT(*)
...
const GetAllTheLoaiService = async ({
  limit,
  page,
  search
}) => {
  try {
    ...
    const {
      rows: theLoaiList,
      count
    } = await db.TheLoai.findAndCountAll({
      attributes: ['id', 'ten_theloai'],
      where: {
        ten_theloai: {
          [Op.like]: `%${search}%`
...
const CreateTheLoaiService = async ({
  ten_theloai
}) => {
  try {
...
const UpdateTheLoaiService = async ({
  id,
  ten_theloai
}) => {
  try {
...
const AddTheLoaiInTruyenService = async ({
  TruyenId,
  TheLoaiIds
}) => {
  try {
    ...
    const results = await Promise.all(TheLoaiIds.map(async TheLoaiId => {
      const truyen = await db.TruyenTheloai.findOne({
        where: {
          TruyenId,
          // Cần kiểm tra TruyenId để chắc chắn thể loại này thuộc về truyện nào
          TheLoaiId
        }
      });
      if (truyen) {
...
const DeleteTheLoaiForTruyenService = async ({
  TruyenId,
  TheLoaiId
}) => {
  try {
...
const CreateChuongService = async ({
  Chuong_so,
  Chuong_ten,
  Chuong_noidung,
  TruyenId
...
const AddImagesChuongService = async ({
  ChuongId,
  images
}) => {
  try {
  ...
  try {
    ...
    const deleteImagePromises = truyenList.map(async truyen => {
      ...
      const deleteChapterImagePromises = chapters.map(async chapter => {
        ...
        await db.chuong_hinhanh.destroy({
          where: {
            ChuongId: chapter.id
          }
        });
...
const DeleteChuongService = async ({
  ChuongId
}) => {
  try {
    const chuong = await db.Chuong.findOne({
...
const DeleteImagesChuongService = async ({
  imagePath
}) => {
  try {
    const image = await db.chuong_hinhanh.findOne({
      where: {
...
const UpdateSortImageService = async ({
  id,
  images
}) => {
  try {
...
const UpdateInfoChuongService = async ({
  id,
  Chuong_so,
  Chuong_ten,
  Chuong_noidung,
