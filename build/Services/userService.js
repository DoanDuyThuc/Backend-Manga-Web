  ...
  try {
    ...
    const isUsername = await db.User.findOne({
      where: {
        ...
        } // Đảm bảo không so sánh với tài khoản hiện tại
      }
    });
    if (isUsername) {
      if (pathAvatar) {
...
const AddThongBaoForUserService = async ({
  idUser,
  content
}) => {
  try {
...
const GetThongBaoForUserService = async ({
  id
}) => {
  try {
...
const DeleteThongBaoForUserService = async ({
  id
}) => {
  try {
