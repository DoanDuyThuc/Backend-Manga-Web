...
const LoginController = async (req, res) => {
  ...
  const account = await LoginService({
    email,
    password
  });
  if (account.error) {
  ...
  res.cookie('refresh_token', refresh_token, {
    httpOnly: true,
    secure: false,
    // Đặt là true nếu bạn sử dụng HTTPS
    sameSite: 'Strict'
  });
...
const SignInController = async (req, res) => {
  ...
  const account = await SignInService({
    username,
    email,
    password
  });
...
const AddThongBaoForUserController = async (req, res) => {
  ...
  const result = await AddThongBaoForUserService({
    idUser,
    content
  });
  if (result.error) {
...
const GetThongBaoForUserController = async (req, res) => {
  ...
  const result = await GetThongBaoForUserService({
    id
  });
  if (result.error) {
    return res.status(400).json(result);
...
const DeleteThongBaoForUserController = async (req, res) => {
  ...
  const result = await DeleteThongBaoForUserService({
    id
  });
  if (result.error) {
    return res.status(400).json(result);
